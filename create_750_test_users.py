...
def get_random_sessions(tournament, num_sessions):
    ...
    # Parse sessions from the tournament's sessions field
    available_sessions = tournament.sessions.split(',') if tournament.sessions else []
    if not available_sessions:
        return []
    
    # Clean up session names
    available_sessions = [s.strip() for s in available_sessions if s.strip()]
...
def create_shipping_address(user):
    ...
    first_name = user.first_name or user.name.split()[0] if user.name else "Test"
    last_name = user.last_name or (user.name.split()[1] if user.name and len(user.name.split()) > 1 else "User")
    
    address = ShippingAddress(
        user_id=user.id,
        first_name=first_name,
        last_name=last_name,
        address_line_1=f"{random.randint(100, 9999)} {random.choice(['Main', 'Oak', 'Pine', 'Elm', 'Cedar', 'Park', 'First', 'Second'])} {random.choice(['St', 'Ave', 'Blvd', 'Dr', 'Way'])}",
...
def create_user_with_tournaments():
    ...
    # Set test_user flag after creating the object
    user.test_user = True
